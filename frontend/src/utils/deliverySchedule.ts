/**
 * Delivery Schedule Utility (Frontend)
 * 
 * Rules:
...
export interface DeliveryScheduleResult {
  ...
  dayNameCapitalized: string; // e.g. 'Wednesday'
  formattedDate: string; // e.g. 'Wednesday, Sep 23'
  deliveryTime: string; // '8:00 AM'
  isAfterCutoff: boolean;
  cutoffTime: string; // '10:00 PM'
  fullScheduleText: string; // 'Wednesday, Sep 23 at 8:00 AM'
}

export const CUTOFF_HOUR = 22; // 10:00 PM
export const DELIVERY_TIME = "8:00 AM";
export const DELIVERY_DAYS = ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday"];

export const getNextDeliverySchedule = (
  orderDate: Date | string | number = new Date(),
  subscribedDays?: string[]
): DeliveryScheduleResult => {
  const dateObj = new Date(orderDate);
  const validDate = isNaN(dateObj.getTime()) ? new Date() : dateObj;

  const currentHour = validDate.getHours();
  const isAfterCutoff = currentHour >= CUTOFF_HOUR;

  // If before 10 PM -> advance 1 day (Tomorrow)
  // If at or after 10 PM -> advance 2 days (Next to next day)
  ...
  return {
    ...
    isAfterCutoff,
    cutoffTime: "10:00 PM",
    fullScheduleText: `${formattedDate} at ${DELIVERY_TIME}`
  };
};

export const getDeliveryScheduleInfo = () => {
  return {
    deliveryTime: DELIVERY_TIME,
    cutoffTime: "10:00 PM",
